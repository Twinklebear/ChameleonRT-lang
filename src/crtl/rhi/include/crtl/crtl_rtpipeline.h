...
CRTL_RHI_EXPORT CRTL_ERROR crtl_new_rtpipeline(CRTLDevice device,
...
// library
CRTL_RHI_EXPORT CRTL_ERROR crtl_set_raygen_record(CRTLDevice device,
                                                  CRTLRTPipeline pipeline,
                                                  CRTLShaderRecord raygen_record);

// Some limitations/things to check here: the miss record has to come from the shader
// library
CRTL_RHI_EXPORT CRTL_ERROR crtl_set_miss_record(CRTLDevice device,
                                                CRTLRTPipeline pipeline,
                                                uint32_t index,
...
CRTL_RHI_EXPORT CRTL_ERROR crtl_set_scene(CRTLDevice device,
                                          CRTLRTPipeline pipeline,
                                          CRTLScene scene);
...
CRTL_RHI_EXPORT CRTL_ERROR crtl_build_shader_table(CRTLDevice device,
                                                   CRTLRTPipeline pipeline);
