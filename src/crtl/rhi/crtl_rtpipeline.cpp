...
crtl_new_rtpipeline(CRTLDevice device,
...
extern "C" CRTL_RHI_EXPORT CRTL_ERROR crtl_set_raygen_record(
    CRTLDevice device, CRTLRTPipeline pipeline, CRTLShaderRecord raygen_record)
{
    crtl::Device *d = reinterpret_cast<crtl::Device *>(device);
    return d->set_raygen_record(pipeline, raygen_record);
}

extern "C" CRTL_RHI_EXPORT CRTL_ERROR crtl_set_miss_record(CRTLDevice device,
                                                           CRTLRTPipeline pipeline,
                                                           uint32_t index,
...
extern "C" CRTL_RHI_EXPORT CRTL_ERROR crtl_set_scene(CRTLDevice device,
                                                     CRTLRTPipeline pipeline,
                                                     CRTLScene scene)
{
    crtl::Device *d = reinterpret_cast<crtl::Device *>(device);
...
extern "C" CRTL_RHI_EXPORT CRTL_ERROR crtl_build_shader_table(CRTLDevice device,
                                                              CRTLRTPipeline pipeline)
{
    crtl::Device *d = reinterpret_cast<crtl::Device *>(device);
...
extern "C" CRTL_RHI_EXPORT CRTL_ERROR crtl_new_global_parameter_block(
